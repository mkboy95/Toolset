import{d as RA,a6 as LA,b as KA,o as Z,f as s,k as x,w as u,g as y,t as L,i as o,a7 as OA,a8 as qA,V as PA,n as $,c as _A,W as AA,l as $A,F as FA,h as AB,A as BB,z as nB,Y as EB}from"./index-be5aec90.js";import{_ as MB}from"./c-key-value-list.vue_vue_type_script_setup_true_lang-cddfe890.js";import{l as O}from"./index-64a9818f.js";import{b as QB,i as IB}from"./index-3aa13386.js";import{r as wB}from"./___vite-browser-external_commonjs-proxy-9eaa9361.js";import{f as DB}from"./convert-ae87c221.js";const gB={flex:"","flex-col":"","gap-2":""},CB={"break-all":"","text-xs":""},NB=RA({__name:"pdf-signature-details",props:{signature:{}},setup(g){const D=g,{signature:C}=LA(D),V={validityPeriod:"Validity period",issuedBy:"Issued by",issuedTo:"Issued to",pemCertificate:"PEM certificate"},c=KA(()=>C.value.meta.certs.map((T,N)=>({...T,validityPeriod:{notBefore:new Date(T.validityPeriod.notBefore).toLocaleString(),notAfter:new Date(T.validityPeriod.notAfter).toLocaleString()},certificateName:`Certificate ${N+1}`})));return(T,N)=>{const Q=MB,Y=OA,h=qA;return Z(),s("div",gB,[x(h,{data:o(c),headers:V},{validityPeriod:u(({value:U})=>[x(Q,{items:[{label:"Not before",value:U.notBefore},{label:"Not after",value:U.notAfter}]},null,8,["items"])]),issuedBy:u(({value:U})=>[x(Q,{items:[{label:"Common name",value:U.commonName},{label:"Organization name",value:U.organizationName},{label:"Country name",value:U.countryName},{label:"Locality name",value:U.localityName},{label:"Organizational unit name",value:U.organizationalUnitName},{label:"State or province name",value:U.stateOrProvinceName}]},null,8,["items"])]),issuedTo:u(({value:U})=>[x(Q,{items:[{label:"Common name",value:U.commonName},{label:"Organization name",value:U.organizationName},{label:"Country name",value:U.countryName},{label:"Locality name",value:U.localityName},{label:"Organizational unit name",value:U.organizationalUnitName},{label:"State or province name",value:U.stateOrProvinceName}]},null,8,["items"])]),pemCertificate:u(({value:U})=>[x(Y,{value:U,label:"View PEM cert"},{value:u(()=>[y("div",CB,L(U),1)]),_:2},1032,["value"])]),_:1},8,["data"])])}}}),iA="TYPE_UNKNOWN",GB="TYPE_INPUT",VB="TYPE_PARSE",TB="TYPE_BYTE_RANGE",FB="VERIFY_SIGNATURE",UB="UNSUPPORTED_SUBFILTER";let lA=class extends Error{constructor(D,C=iA){super(D),this.type=C}};Object.assign(lA,{TYPE_UNKNOWN:iA,TYPE_INPUT:GB,TYPE_PARSE:VB,TYPE_BYTE_RANGE:TB,VERIFY_SIGNATURE:FB,UNSUPPORTED_SUBFILTER:UB});var nA=lA,EA={};(function(g){var D=QB,C=IB,V=typeof Symbol=="function"&&typeof Symbol.for=="function"?Symbol.for("nodejs.util.inspect.custom"):null;g.Buffer=Q,g.SlowBuffer=QA,g.INSPECT_MAX_BYTES=50;var c=2147483647;g.kMaxLength=c,Q.TYPED_ARRAY_SUPPORT=T(),!Q.TYPED_ARRAY_SUPPORT&&typeof console<"u"&&typeof console.error=="function"&&console.error("This browser lacks typed array (Uint8Array) support which is required by `buffer` v5.x. Use `buffer` v4.x if you require old browser support.");function T(){try{var n=new Uint8Array(1),A={foo:function(){return 42}};return Object.setPrototypeOf(A,Uint8Array.prototype),Object.setPrototypeOf(n,A),n.foo()===42}catch{return!1}}Object.defineProperty(Q.prototype,"parent",{enumerable:!0,get:function(){if(Q.isBuffer(this))return this.buffer}}),Object.defineProperty(Q.prototype,"offset",{enumerable:!0,get:function(){if(Q.isBuffer(this))return this.byteOffset}});function N(n){if(n>c)throw new RangeError('The value "'+n+'" is invalid for option "size"');var A=new Uint8Array(n);return Object.setPrototypeOf(A,Q.prototype),A}function Q(n,A,B){if(typeof n=="number"){if(typeof A=="string")throw new TypeError('The "string" argument must be of type string. Received type number');return b(n)}return Y(n,A,B)}Q.poolSize=8192;function Y(n,A,B){if(typeof n=="string")return j(n,A);if(ArrayBuffer.isView(n))return S(n);if(n==null)throw new TypeError("The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type "+typeof n);if(d(n,ArrayBuffer)||n&&d(n.buffer,ArrayBuffer)||typeof SharedArrayBuffer<"u"&&(d(n,SharedArrayBuffer)||n&&d(n.buffer,SharedArrayBuffer)))return k(n,A,B);if(typeof n=="number")throw new TypeError('The "value" argument must not be of type number. Received type number');var E=n.valueOf&&n.valueOf();if(E!=null&&E!==n)return Q.from(E,A,B);var M=MA(n);if(M)return M;if(typeof Symbol<"u"&&Symbol.toPrimitive!=null&&typeof n[Symbol.toPrimitive]=="function")return Q.from(n[Symbol.toPrimitive]("string"),A,B);throw new TypeError("The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type "+typeof n)}Q.from=function(n,A,B){return Y(n,A,B)},Object.setPrototypeOf(Q.prototype,Uint8Array.prototype),Object.setPrototypeOf(Q,Uint8Array);function h(n){if(typeof n!="number")throw new TypeError('"size" argument must be of type number');if(n<0)throw new RangeError('The value "'+n+'" is invalid for option "size"')}function U(n,A,B){return h(n),n<=0?N(n):A!==void 0?typeof B=="string"?N(n).fill(A,B):N(n).fill(A):N(n)}Q.alloc=function(n,A,B){return U(n,A,B)};function b(n){return h(n),N(n<0?0:m(n)|0)}Q.allocUnsafe=function(n){return b(n)},Q.allocUnsafeSlow=function(n){return b(n)};function j(n,A){if((typeof A!="string"||A==="")&&(A="utf8"),!Q.isEncoding(A))throw new TypeError("Unknown encoding: "+A);var B=H(n,A)|0,E=N(B),M=E.write(n,A);return M!==B&&(E=E.slice(0,M)),E}function S(n){for(var A=n.length<0?0:m(n.length)|0,B=N(A),E=0;E<A;E+=1)B[E]=n[E]&255;return B}function k(n,A,B){if(A<0||n.byteLength<A)throw new RangeError('"offset" is outside of buffer bounds');if(n.byteLength<A+(B||0))throw new RangeError('"length" is outside of buffer bounds');var E;return A===void 0&&B===void 0?E=new Uint8Array(n):B===void 0?E=new Uint8Array(n,A):E=new Uint8Array(n,A,B),Object.setPrototypeOf(E,Q.prototype),E}function MA(n){if(Q.isBuffer(n)){var A=m(n.length)|0,B=N(A);return B.length===0||n.copy(B,0,0,A),B}if(n.length!==void 0)return typeof n.length!="number"||_(n.length)?N(0):S(n);if(n.type==="Buffer"&&Array.isArray(n.data))return S(n.data)}function m(n){if(n>=c)throw new RangeError("Attempt to allocate Buffer larger than maximum size: 0x"+c.toString(16)+" bytes");return n|0}function QA(n){return+n!=n&&(n=0),Q.alloc(+n)}Q.isBuffer=function(A){return A!=null&&A._isBuffer===!0&&A!==Q.prototype},Q.compare=function(A,B){if(d(A,Uint8Array)&&(A=Q.from(A,A.offset,A.byteLength)),d(B,Uint8Array)&&(B=Q.from(B,B.offset,B.byteLength)),!Q.isBuffer(A)||!Q.isBuffer(B))throw new TypeError('The "buf1", "buf2" arguments must be one of type Buffer or Uint8Array');if(A===B)return 0;for(var E=A.length,M=B.length,I=0,w=Math.min(E,M);I<w;++I)if(A[I]!==B[I]){E=A[I],M=B[I];break}return E<M?-1:M<E?1:0},Q.isEncoding=function(A){switch(String(A).toLowerCase()){case"hex":case"utf8":case"utf-8":case"ascii":case"latin1":case"binary":case"base64":case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return!0;default:return!1}},Q.concat=function(A,B){if(!Array.isArray(A))throw new TypeError('"list" argument must be an Array of Buffers');if(A.length===0)return Q.alloc(0);var E;if(B===void 0)for(B=0,E=0;E<A.length;++E)B+=A[E].length;var M=Q.allocUnsafe(B),I=0;for(E=0;E<A.length;++E){var w=A[E];if(d(w,Uint8Array)&&(w=Q.from(w)),!Q.isBuffer(w))throw new TypeError('"list" argument must be an Array of Buffers');w.copy(M,I),I+=w.length}return M};function H(n,A){if(Q.isBuffer(n))return n.length;if(ArrayBuffer.isView(n)||d(n,ArrayBuffer))return n.byteLength;if(typeof n!="string")throw new TypeError('The "string" argument must be one of type string, Buffer, or ArrayBuffer. Received type '+typeof n);var B=n.length,E=arguments.length>2&&arguments[2]===!0;if(!E&&B===0)return 0;for(var M=!1;;)switch(A){case"ascii":case"latin1":case"binary":return B;case"utf8":case"utf-8":return P(n).length;case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return B*2;case"hex":return B>>>1;case"base64":return GA(n).length;default:if(M)return E?-1:P(n).length;A=(""+A).toLowerCase(),M=!0}}Q.byteLength=H;function J(n,A,B){var E=!1;if((A===void 0||A<0)&&(A=0),A>this.length||((B===void 0||B>this.length)&&(B=this.length),B<=0)||(B>>>=0,A>>>=0,B<=A))return"";for(n||(n="utf8");;)switch(n){case"hex":return WA(this,A,B);case"utf8":case"utf-8":return wA(this,A,B);case"ascii":return HA(this,A,B);case"latin1":case"binary":return JA(this,A,B);case"base64":return zA(this,A,B);case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return pA(this,A,B);default:if(E)throw new TypeError("Unknown encoding: "+n);n=(n+"").toLowerCase(),E=!0}}Q.prototype._isBuffer=!0;function a(n,A,B){var E=n[A];n[A]=n[B],n[B]=E}Q.prototype.swap16=function(){var A=this.length;if(A%2!==0)throw new RangeError("Buffer size must be a multiple of 16-bits");for(var B=0;B<A;B+=2)a(this,B,B+1);return this},Q.prototype.swap32=function(){var A=this.length;if(A%4!==0)throw new RangeError("Buffer size must be a multiple of 32-bits");for(var B=0;B<A;B+=4)a(this,B,B+3),a(this,B+1,B+2);return this},Q.prototype.swap64=function(){var A=this.length;if(A%8!==0)throw new RangeError("Buffer size must be a multiple of 64-bits");for(var B=0;B<A;B+=8)a(this,B,B+7),a(this,B+1,B+6),a(this,B+2,B+5),a(this,B+3,B+4);return this},Q.prototype.toString=function(){var A=this.length;return A===0?"":arguments.length===0?wA(this,0,A):J.apply(this,arguments)},Q.prototype.toLocaleString=Q.prototype.toString,Q.prototype.equals=function(A){if(!Q.isBuffer(A))throw new TypeError("Argument must be a Buffer");return this===A?!0:Q.compare(this,A)===0},Q.prototype.inspect=function(){var A="",B=g.INSPECT_MAX_BYTES;return A=this.toString("hex",0,B).replace(/(.{2})/g,"$1 ").trim(),this.length>B&&(A+=" ... "),"<Buffer "+A+">"},V&&(Q.prototype[V]=Q.prototype.inspect),Q.prototype.compare=function(A,B,E,M,I){if(d(A,Uint8Array)&&(A=Q.from(A,A.offset,A.byteLength)),!Q.isBuffer(A))throw new TypeError('The "target" argument must be one of type Buffer or Uint8Array. Received type '+typeof A);if(B===void 0&&(B=0),E===void 0&&(E=A?A.length:0),M===void 0&&(M=0),I===void 0&&(I=this.length),B<0||E>A.length||M<0||I>this.length)throw new RangeError("out of range index");if(M>=I&&B>=E)return 0;if(M>=I)return-1;if(B>=E)return 1;if(B>>>=0,E>>>=0,M>>>=0,I>>>=0,this===A)return 0;for(var w=I-M,G=E-B,F=Math.min(w,G),t=this.slice(M,I),i=A.slice(B,E),r=0;r<F;++r)if(t[r]!==i[r]){w=t[r],G=i[r];break}return w<G?-1:G<w?1:0};function W(n,A,B,E,M){if(n.length===0)return-1;if(typeof B=="string"?(E=B,B=0):B>2147483647?B=2147483647:B<-2147483648&&(B=-2147483648),B=+B,_(B)&&(B=M?0:n.length-1),B<0&&(B=n.length+B),B>=n.length){if(M)return-1;B=n.length-1}else if(B<0)if(M)B=0;else return-1;if(typeof A=="string"&&(A=Q.from(A,E)),Q.isBuffer(A))return A.length===0?-1:p(n,A,B,E,M);if(typeof A=="number")return A=A&255,typeof Uint8Array.prototype.indexOf=="function"?M?Uint8Array.prototype.indexOf.call(n,A,B):Uint8Array.prototype.lastIndexOf.call(n,A,B):p(n,[A],B,E,M);throw new TypeError("val must be string, number or Buffer")}function p(n,A,B,E,M){var I=1,w=n.length,G=A.length;if(E!==void 0&&(E=String(E).toLowerCase(),E==="ucs2"||E==="ucs-2"||E==="utf16le"||E==="utf-16le")){if(n.length<2||A.length<2)return-1;I=2,w/=2,G/=2,B/=2}function F(VA,TA){return I===1?VA[TA]:VA.readUInt16BE(TA*I)}var t;if(M){var i=-1;for(t=B;t<w;t++)if(F(n,t)===F(A,i===-1?0:t-i)){if(i===-1&&(i=t),t-i+1===G)return i*I}else i!==-1&&(t-=t-i),i=-1}else for(B+G>w&&(B=w-G),t=B;t>=0;t--){for(var r=!0,v=0;v<G;v++)if(F(n,t+v)!==F(A,v)){r=!1;break}if(r)return t}return-1}Q.prototype.includes=function(A,B,E){return this.indexOf(A,B,E)!==-1},Q.prototype.indexOf=function(A,B,E){return W(this,A,B,E,!0)},Q.prototype.lastIndexOf=function(A,B,E){return W(this,A,B,E,!1)};function q(n,A,B,E){B=Number(B)||0;var M=n.length-B;E?(E=Number(E),E>M&&(E=M)):E=M;var I=A.length;E>I/2&&(E=I/2);for(var w=0;w<E;++w){var G=parseInt(A.substr(w*2,2),16);if(_(G))return w;n[B+w]=G}return w}function uA(n,A,B,E){return X(P(A,n.length-B),n,B,E)}function IA(n,A,B,E){return X(XA(A),n,B,E)}function jA(n,A,B,E){return IA(n,A,B,E)}function sA(n,A,B,E){return X(GA(A),n,B,E)}function yA(n,A,B,E){return X(vA(A,n.length-B),n,B,E)}Q.prototype.write=function(A,B,E,M){if(B===void 0)M="utf8",E=this.length,B=0;else if(E===void 0&&typeof B=="string")M=B,E=this.length,B=0;else if(isFinite(B))B=B>>>0,isFinite(E)?(E=E>>>0,M===void 0&&(M="utf8")):(M=E,E=void 0);else throw new Error("Buffer.write(string, encoding, offset[, length]) is no longer supported");var I=this.length-B;if((E===void 0||E>I)&&(E=I),A.length>0&&(E<0||B<0)||B>this.length)throw new RangeError("Attempt to write outside buffer bounds");M||(M="utf8");for(var w=!1;;)switch(M){case"hex":return q(this,A,B,E);case"utf8":case"utf-8":return uA(this,A,B,E);case"ascii":return IA(this,A,B,E);case"latin1":case"binary":return jA(this,A,B,E);case"base64":return sA(this,A,B,E);case"ucs2":case"ucs-2":case"utf16le":case"utf-16le":return yA(this,A,B,E);default:if(w)throw new TypeError("Unknown encoding: "+M);M=(""+M).toLowerCase(),w=!0}},Q.prototype.toJSON=function(){return{type:"Buffer",data:Array.prototype.slice.call(this._arr||this,0)}};function zA(n,A,B){return A===0&&B===n.length?D.fromByteArray(n):D.fromByteArray(n.slice(A,B))}function wA(n,A,B){B=Math.min(n.length,B);for(var E=[],M=A;M<B;){var I=n[M],w=null,G=I>239?4:I>223?3:I>191?2:1;if(M+G<=B){var F,t,i,r;switch(G){case 1:I<128&&(w=I);break;case 2:F=n[M+1],(F&192)===128&&(r=(I&31)<<6|F&63,r>127&&(w=r));break;case 3:F=n[M+1],t=n[M+2],(F&192)===128&&(t&192)===128&&(r=(I&15)<<12|(F&63)<<6|t&63,r>2047&&(r<55296||r>57343)&&(w=r));break;case 4:F=n[M+1],t=n[M+2],i=n[M+3],(F&192)===128&&(t&192)===128&&(i&192)===128&&(r=(I&15)<<18|(F&63)<<12|(t&63)<<6|i&63,r>65535&&r<1114112&&(w=r))}}w===null?(w=65533,G=1):w>65535&&(w-=65536,E.push(w>>>10&1023|55296),w=56320|w&1023),E.push(w),M+=G}return mA(E)}var DA=4096;function mA(n){var A=n.length;if(A<=DA)return String.fromCharCode.apply(String,n);for(var B="",E=0;E<A;)B+=String.fromCharCode.apply(String,n.slice(E,E+=DA));return B}function HA(n,A,B){var E="";B=Math.min(n.length,B);for(var M=A;M<B;++M)E+=String.fromCharCode(n[M]&127);return E}function JA(n,A,B){var E="";B=Math.min(n.length,B);for(var M=A;M<B;++M)E+=String.fromCharCode(n[M]);return E}function WA(n,A,B){var E=n.length;(!A||A<0)&&(A=0),(!B||B<0||B>E)&&(B=E);for(var M="",I=A;I<B;++I)M+=fA[n[I]];return M}function pA(n,A,B){for(var E=n.slice(A,B),M="",I=0;I<E.length;I+=2)M+=String.fromCharCode(E[I]+E[I+1]*256);return M}Q.prototype.slice=function(A,B){var E=this.length;A=~~A,B=B===void 0?E:~~B,A<0?(A+=E,A<0&&(A=0)):A>E&&(A=E),B<0?(B+=E,B<0&&(B=0)):B>E&&(B=E),B<A&&(B=A);var M=this.subarray(A,B);return Object.setPrototypeOf(M,Q.prototype),M};function R(n,A,B){if(n%1!==0||n<0)throw new RangeError("offset is not uint");if(n+A>B)throw new RangeError("Trying to access beyond buffer length")}Q.prototype.readUIntLE=function(A,B,E){A=A>>>0,B=B>>>0,E||R(A,B,this.length);for(var M=this[A],I=1,w=0;++w<B&&(I*=256);)M+=this[A+w]*I;return M},Q.prototype.readUIntBE=function(A,B,E){A=A>>>0,B=B>>>0,E||R(A,B,this.length);for(var M=this[A+--B],I=1;B>0&&(I*=256);)M+=this[A+--B]*I;return M},Q.prototype.readUInt8=function(A,B){return A=A>>>0,B||R(A,1,this.length),this[A]},Q.prototype.readUInt16LE=function(A,B){return A=A>>>0,B||R(A,2,this.length),this[A]|this[A+1]<<8},Q.prototype.readUInt16BE=function(A,B){return A=A>>>0,B||R(A,2,this.length),this[A]<<8|this[A+1]},Q.prototype.readUInt32LE=function(A,B){return A=A>>>0,B||R(A,4,this.length),(this[A]|this[A+1]<<8|this[A+2]<<16)+this[A+3]*16777216},Q.prototype.readUInt32BE=function(A,B){return A=A>>>0,B||R(A,4,this.length),this[A]*16777216+(this[A+1]<<16|this[A+2]<<8|this[A+3])},Q.prototype.readIntLE=function(A,B,E){A=A>>>0,B=B>>>0,E||R(A,B,this.length);for(var M=this[A],I=1,w=0;++w<B&&(I*=256);)M+=this[A+w]*I;return I*=128,M>=I&&(M-=Math.pow(2,8*B)),M},Q.prototype.readIntBE=function(A,B,E){A=A>>>0,B=B>>>0,E||R(A,B,this.length);for(var M=B,I=1,w=this[A+--M];M>0&&(I*=256);)w+=this[A+--M]*I;return I*=128,w>=I&&(w-=Math.pow(2,8*B)),w},Q.prototype.readInt8=function(A,B){return A=A>>>0,B||R(A,1,this.length),this[A]&128?(255-this[A]+1)*-1:this[A]},Q.prototype.readInt16LE=function(A,B){A=A>>>0,B||R(A,2,this.length);var E=this[A]|this[A+1]<<8;return E&32768?E|4294901760:E},Q.prototype.readInt16BE=function(A,B){A=A>>>0,B||R(A,2,this.length);var E=this[A+1]|this[A]<<8;return E&32768?E|4294901760:E},Q.prototype.readInt32LE=function(A,B){return A=A>>>0,B||R(A,4,this.length),this[A]|this[A+1]<<8|this[A+2]<<16|this[A+3]<<24},Q.prototype.readInt32BE=function(A,B){return A=A>>>0,B||R(A,4,this.length),this[A]<<24|this[A+1]<<16|this[A+2]<<8|this[A+3]},Q.prototype.readFloatLE=function(A,B){return A=A>>>0,B||R(A,4,this.length),C.read(this,A,!0,23,4)},Q.prototype.readFloatBE=function(A,B){return A=A>>>0,B||R(A,4,this.length),C.read(this,A,!1,23,4)},Q.prototype.readDoubleLE=function(A,B){return A=A>>>0,B||R(A,8,this.length),C.read(this,A,!0,52,8)},Q.prototype.readDoubleBE=function(A,B){return A=A>>>0,B||R(A,8,this.length),C.read(this,A,!1,52,8)};function l(n,A,B,E,M,I){if(!Q.isBuffer(n))throw new TypeError('"buffer" argument must be a Buffer instance');if(A>M||A<I)throw new RangeError('"value" argument is out of bounds');if(B+E>n.length)throw new RangeError("Index out of range")}Q.prototype.writeUIntLE=function(A,B,E,M){if(A=+A,B=B>>>0,E=E>>>0,!M){var I=Math.pow(2,8*E)-1;l(this,A,B,E,I,0)}var w=1,G=0;for(this[B]=A&255;++G<E&&(w*=256);)this[B+G]=A/w&255;return B+E},Q.prototype.writeUIntBE=function(A,B,E,M){if(A=+A,B=B>>>0,E=E>>>0,!M){var I=Math.pow(2,8*E)-1;l(this,A,B,E,I,0)}var w=E-1,G=1;for(this[B+w]=A&255;--w>=0&&(G*=256);)this[B+w]=A/G&255;return B+E},Q.prototype.writeUInt8=function(A,B,E){return A=+A,B=B>>>0,E||l(this,A,B,1,255,0),this[B]=A&255,B+1},Q.prototype.writeUInt16LE=function(A,B,E){return A=+A,B=B>>>0,E||l(this,A,B,2,65535,0),this[B]=A&255,this[B+1]=A>>>8,B+2},Q.prototype.writeUInt16BE=function(A,B,E){return A=+A,B=B>>>0,E||l(this,A,B,2,65535,0),this[B]=A>>>8,this[B+1]=A&255,B+2},Q.prototype.writeUInt32LE=function(A,B,E){return A=+A,B=B>>>0,E||l(this,A,B,4,4294967295,0),this[B+3]=A>>>24,this[B+2]=A>>>16,this[B+1]=A>>>8,this[B]=A&255,B+4},Q.prototype.writeUInt32BE=function(A,B,E){return A=+A,B=B>>>0,E||l(this,A,B,4,4294967295,0),this[B]=A>>>24,this[B+1]=A>>>16,this[B+2]=A>>>8,this[B+3]=A&255,B+4},Q.prototype.writeIntLE=function(A,B,E,M){if(A=+A,B=B>>>0,!M){var I=Math.pow(2,8*E-1);l(this,A,B,E,I-1,-I)}var w=0,G=1,F=0;for(this[B]=A&255;++w<E&&(G*=256);)A<0&&F===0&&this[B+w-1]!==0&&(F=1),this[B+w]=(A/G>>0)-F&255;return B+E},Q.prototype.writeIntBE=function(A,B,E,M){if(A=+A,B=B>>>0,!M){var I=Math.pow(2,8*E-1);l(this,A,B,E,I-1,-I)}var w=E-1,G=1,F=0;for(this[B+w]=A&255;--w>=0&&(G*=256);)A<0&&F===0&&this[B+w+1]!==0&&(F=1),this[B+w]=(A/G>>0)-F&255;return B+E},Q.prototype.writeInt8=function(A,B,E){return A=+A,B=B>>>0,E||l(this,A,B,1,127,-128),A<0&&(A=255+A+1),this[B]=A&255,B+1},Q.prototype.writeInt16LE=function(A,B,E){return A=+A,B=B>>>0,E||l(this,A,B,2,32767,-32768),this[B]=A&255,this[B+1]=A>>>8,B+2},Q.prototype.writeInt16BE=function(A,B,E){return A=+A,B=B>>>0,E||l(this,A,B,2,32767,-32768),this[B]=A>>>8,this[B+1]=A&255,B+2},Q.prototype.writeInt32LE=function(A,B,E){return A=+A,B=B>>>0,E||l(this,A,B,4,2147483647,-2147483648),this[B]=A&255,this[B+1]=A>>>8,this[B+2]=A>>>16,this[B+3]=A>>>24,B+4},Q.prototype.writeInt32BE=function(A,B,E){return A=+A,B=B>>>0,E||l(this,A,B,4,2147483647,-2147483648),A<0&&(A=4294967295+A+1),this[B]=A>>>24,this[B+1]=A>>>16,this[B+2]=A>>>8,this[B+3]=A&255,B+4};function gA(n,A,B,E,M,I){if(B+E>n.length)throw new RangeError("Index out of range");if(B<0)throw new RangeError("Index out of range")}function CA(n,A,B,E,M){return A=+A,B=B>>>0,M||gA(n,A,B,4),C.write(n,A,B,E,23,4),B+4}Q.prototype.writeFloatLE=function(A,B,E){return CA(this,A,B,!0,E)},Q.prototype.writeFloatBE=function(A,B,E){return CA(this,A,B,!1,E)};function NA(n,A,B,E,M){return A=+A,B=B>>>0,M||gA(n,A,B,8),C.write(n,A,B,E,52,8),B+8}Q.prototype.writeDoubleLE=function(A,B,E){return NA(this,A,B,!0,E)},Q.prototype.writeDoubleBE=function(A,B,E){return NA(this,A,B,!1,E)},Q.prototype.copy=function(A,B,E,M){if(!Q.isBuffer(A))throw new TypeError("argument should be a Buffer");if(E||(E=0),!M&&M!==0&&(M=this.length),B>=A.length&&(B=A.length),B||(B=0),M>0&&M<E&&(M=E),M===E||A.length===0||this.length===0)return 0;if(B<0)throw new RangeError("targetStart out of bounds");if(E<0||E>=this.length)throw new RangeError("Index out of range");if(M<0)throw new RangeError("sourceEnd out of bounds");M>this.length&&(M=this.length),A.length-B<M-E&&(M=A.length-B+E);var I=M-E;if(this===A&&typeof Uint8Array.prototype.copyWithin=="function")this.copyWithin(B,E,M);else if(this===A&&E<B&&B<M)for(var w=I-1;w>=0;--w)A[w+B]=this[w+E];else Uint8Array.prototype.set.call(A,this.subarray(E,M),B);return I},Q.prototype.fill=function(A,B,E,M){if(typeof A=="string"){if(typeof B=="string"?(M=B,B=0,E=this.length):typeof E=="string"&&(M=E,E=this.length),M!==void 0&&typeof M!="string")throw new TypeError("encoding must be a string");if(typeof M=="string"&&!Q.isEncoding(M))throw new TypeError("Unknown encoding: "+M);if(A.length===1){var I=A.charCodeAt(0);(M==="utf8"&&I<128||M==="latin1")&&(A=I)}}else typeof A=="number"?A=A&255:typeof A=="boolean"&&(A=Number(A));if(B<0||this.length<B||this.length<E)throw new RangeError("Out of range index");if(E<=B)return this;B=B>>>0,E=E===void 0?this.length:E>>>0,A||(A=0);var w;if(typeof A=="number")for(w=B;w<E;++w)this[w]=A;else{var G=Q.isBuffer(A)?A:Q.from(A,M),F=G.length;if(F===0)throw new TypeError('The value "'+A+'" is invalid for argument "value"');for(w=0;w<E-B;++w)this[w+B]=G[w%F]}return this};var SA=/[^+/0-9A-Za-z-_]/g;function kA(n){if(n=n.split("=")[0],n=n.trim().replace(SA,""),n.length<2)return"";for(;n.length%4!==0;)n=n+"=";return n}function P(n,A){A=A||1/0;for(var B,E=n.length,M=null,I=[],w=0;w<E;++w){if(B=n.charCodeAt(w),B>55295&&B<57344){if(!M){if(B>56319){(A-=3)>-1&&I.push(239,191,189);continue}else if(w+1===E){(A-=3)>-1&&I.push(239,191,189);continue}M=B;continue}if(B<56320){(A-=3)>-1&&I.push(239,191,189),M=B;continue}B=(M-55296<<10|B-56320)+65536}else M&&(A-=3)>-1&&I.push(239,191,189);if(M=null,B<128){if((A-=1)<0)break;I.push(B)}else if(B<2048){if((A-=2)<0)break;I.push(B>>6|192,B&63|128)}else if(B<65536){if((A-=3)<0)break;I.push(B>>12|224,B>>6&63|128,B&63|128)}else if(B<1114112){if((A-=4)<0)break;I.push(B>>18|240,B>>12&63|128,B>>6&63|128,B&63|128)}else throw new Error("Invalid code point")}return I}function XA(n){for(var A=[],B=0;B<n.length;++B)A.push(n.charCodeAt(B)&255);return A}function vA(n,A){for(var B,E,M,I=[],w=0;w<n.length&&!((A-=2)<0);++w)B=n.charCodeAt(w),E=B>>8,M=B%256,I.push(M),I.push(E);return I}function GA(n){return D.toByteArray(kA(n))}function X(n,A,B,E){for(var M=0;M<E&&!(M+B>=A.length||M>=n.length);++M)A[M+B]=n[M];return M}function d(n,A){return n instanceof A||n!=null&&n.constructor!=null&&n.constructor.name!=null&&n.constructor.name===A.name}function _(n){return n!==n}var fA=function(){for(var n="0123456789abcdef",A=new Array(256),B=0;B<16;++B)for(var E=B*16,M=0;M<16;++M)A[E+M]=n[B]+n[M];return A}()})(EA);const UA=O,{Buffer:BA}=EA,z=nA,tB=g=>{try{return BA.isBuffer(g)?g:BA.from(g)}catch{throw new z("PDF expected as Buffer.",z.TYPE_INPUT)}},cB=g=>{const D=g.toString().match(/\/SubFilter\s*\/([\w.]*)/),C=Array.isArray(D)&&D[1];if(!C)throw new z("cannot find subfilter",z.TYPE_PARSE);if(!["adbe.pkcs7.detached","etsi.cades.detached"].includes(C.trim().toLowerCase()))throw new z(`subFilter ${C} not supported`,z.UNSUPPORTED_SUBFILTER)},rB=g=>{const D=UA.asn1.fromDer(g,{parseAllBytes:!1});return UA.pkcs7.messageFromAsn1(D)},f=g=>D=>{const C=new RegExp(`/${g}\\s*\\(([\\w.\\s@,]*)`,"g"),V=[...D.matchAll(C)];return V.length?V[V.length-1][1]:null},YB=g=>{const D=BA.isBuffer(g)?g.toString():g;return{reason:f("Reason")(D),contactInfo:f("ContactInfo")(D),location:f("Location")(D),name:f("Name")(D)}};var hA={checkForSubFilter:cB,getSignatureMeta:YB,getMessageFromSignature:rB,preparePDF:tB};const{Buffer:tA}=EA,K=nA,{getSignatureMeta:RB,preparePDF:iB}=hA,lB="**********",hB=g=>{const D=g.toString().match(/\/ByteRange\s*\[{1}\s*(?:(?:\d*|\/\*{10})\s+){3}(?:\d+|\/\*{10}){1}\s*\]{1}/g);if(!D)throw new K("Failed to locate ByteRange.",K.TYPE_PARSE);const C=D.find(T=>T.includes(`/${lB}`)),c=D.map(T=>T.match(/[^[\s]*(?:\d|\/\*{10})/g)).map(T=>T.map(Number));return{byteRangePlaceholder:C,byteRanges:c}},eB=g=>{const D=iB(g),{byteRanges:C}=hB(D),V=C.length-1,c=C[V][2]+C[V][3];if(D.length>c)throw new K("Failed byte range verification.",K.VERIFY_BYTE_RANGE);const T=[],N=[];C.forEach(Y=>{N.push(tA.concat([D.slice(Y[0],Y[0]+Y[1]),D.slice(Y[2],Y[2]+Y[3])]));const h=D.slice(Y[0]+Y[1]+1,Y[2]).toString("latin1");T.push(tA.from(h,"hex").toString("latin1"))});const Q=N.map(Y=>RB(Y));return{byteRanges:C,signatureStr:T,signedData:N,signatureMeta:Q}};var aB=eB;const dB=g=>D=>g!==D&&D.issued(g),oB=g=>D=>g.find(dB(D)),xB=g=>D=>!g(D),bB=g=>xB(oB(g)),ZB=g=>g.findIndex(bB(g)),uB=g=>D=>g!==D&&g.issued(D),jB=g=>D=>g.findIndex(uB(D)),eA=g=>{const D=Array.from(g),C=ZB(D),V=D.splice(C,1);for(;D.length;){const c=V[0],T=jB(D)(c);if(T===-1)D.splice(T,1);else{const[N]=D.splice(T,1);V.unshift(N)}}return V},sB=g=>eA(g)[0];var yB={sortCertificateChain:eA,getClientCertificate:sB};const zB=[`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

R9I4LtD+gdwyah617jzV/OeBHRnDJELqYzmp
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

j2A781q0tHuu2guQOHXvgR1m0vdXcDazv/wor3ElhVsT/h5/WrQ8
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

akhT2+zNVVXxxvjpoixMptEmX36vWkzaH6byHCx+rgIW0lbQL1dTR+iS
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

ZdWCEOrCMc0u
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

V3bqxbYoOb8VZRzI9neWagqNdwvYkQsEjgfbKbYK7p2CNTUQ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

W8mw0FfB+j564ZfJ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

5Aq7KkzrCWA5zspi2C5u
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

ThaA63p4ZUWiABqvDA1VZDRIuJK58bRQKfJPIx/abKwfROHdI3hRW8cW
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

NmdspZS11My5vWo1ViHe2MPr+8ukYEywVaCge1ey
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

Ecgh4LMRkWXbtKaIOM5V
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

aZVDADlN9u6wWk5JRFRYX0KD
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

9vuD4DPTCF+hnMJbn0bWtsuRBmOiBuczrD6ogRLQy7rQkgu2npaqBA+K
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

LXpUq3DDfSJlgnCW
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

WD9f
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

8zsxkVrctQgVrXYlCg17LofiDKYGvCYQbTed7N14jHyAxfDZd0jQ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

SC34MCN32EZLeW32jO06f2ARePTpm67VVMB0gNELQp/B
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

NnReJaH1ZgUufzkVqSr7UIuOhWn0
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

/Wi3MOxw+3WKt21gZ7IeyLnp2KhvAotnDU0mV3HaIPzBSlCNsSi6
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

SjnRBUkLp7Y3gaVdjKozXoEofKd9J+sAro03
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

JuVvw9y4AyHqnxbxLFS1
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

QCj8KWefshNPZiTEUxnpHikV7+ZtsH8tZ/3zbBt1RqPlShfppNcL
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

D0tVNEYqi4Y7
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

KrcYPqcZ2Qt9sTdBQrC6YB3y/gkRsPCHe6ed
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

tIsPm6Ik8KaN1nrgS7ZklmOVhMJKzRwuJIczYOXD
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

iC3oheGe7IUIarFsNMkd7EgrO3jtZsSOeWmD3n+M
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

WGWVjUTR939+J399roD1B0y2PpxxVJkES/1Y+Zj0
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

3KjVSaLtx9zRSX8XAbjIho9OjIgrqJqpisXRAL34VOKa5Vt8sycX
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

6hN35N6MgSGtBxBHEa2HPQfRdbzP82Z+
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

88Tk5c0NqMuRkrF8hey1FGlmDoLnzc7ILaZRfyHBNVOFBkpdn627G190
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

+SvzZpA3
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

pRtK4dw4LTzcqx+QGtVKnO7RcGzM7vRX+Bi6hG6H
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

BOxonbRJRBD0ckscZOf85muQ3Wl9af0AVqW3rLatt8o+Ae+c
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

WyVgrtBIGu4G
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

Kc2Jo5YPSjXnTkLAdc0Hz+Ys63su
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

w5Khchf8R7agCyzxxN5DaAhqXzvwdmP7zAYspsbiDrW5viSP
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

qiAeW2GFZECpkJcNrVPSWh2HagCXZWK0vm9qp/UsQu0yrbYhnr68
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

m8rifOoCWCKR
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

eGADbkpyrqXRfboQnoZsG4q5WTP468SQvvG5
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

KuGUaJPHZnJED4AhMmwlxyOAgwrr
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

wpaYT05KnJe32x+SMsj/D1Fu1uwJ
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

0nxpbl/f5Wpl
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

i9fgojaGHAeCOnZT/cKi7e97sIBPWA9LUzm9
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

cu51OyLemGhmW/HGY0dVHLqlCFF1pkgl
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

XS/Dr55FBcOEArf9LAhST4Ldo/DUhgkC
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

UfUk7GRSJFClH9voy8l27OyCbvWFGFPouOOaKaqW04MjyaR7YbPMAuhd
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

NlK73nZfdklJrX+9ZSCyycErdhh2n1ax
-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----`,`-----BEGIN CERTIFICATE-----

UvToiIMrVCjU8jVStDKDYmlkDJGcn5fqdBb9HxEGmpv0
-----END CERTIFICATE-----`],mB=wB,cA=O,HB=zB,JB=()=>mB.rootCertificates||HB,aA=g=>!!JB().find(D=>{try{const C=cA.pki.certificateFromPem(D);return cA.pki.certificateToPem(g)===D||C.issued(g)}catch{return!1}}),dA=g=>!!g.find((D,C)=>g[C+1]&&g[C+1].issued(D)),WB=g=>!!g.find(({validity:{notAfter:D,notBefore:C}})=>D.getTime()<Date.now()||C.getTime()>Date.now()),pB=g=>dA(g)&&aA(g[g.length-1]);var SB={authenticateSignature:pB,verifyCaBundle:dA,verifyRootCert:aA,isCertsExpired:WB};const kB=aB,XB=yB,vB=hA,fB=SB;var oA={extractSignature:kB,...XB,...vB,...fB};const LB=O,{extractSignature:KB,getMessageFromSignature:OB,preparePDF:qB}=oA,rA=g=>g.reduce((D,{name:C,value:V})=>(C&&(D[C]=V),D),{}),PB=g=>{const{issuer:D,subject:C,validity:V}=g;return{issuedBy:rA(D.attributes),issuedTo:rA(C.attributes),validityPeriod:V,pemCertificate:LB.pki.certificateToPem(g)}},xA=g=>g.map(PB).map((D,C)=>C?D:{clientCertificate:!0,...D}),_B=g=>{const D=qB(g),{signatureStr:C}=KB(D);return C.map(V=>{const{certificates:c}=OB(V);return xA(c)})};var bA={extractCertificatesDetails:xA,getCertificatesInfoFromPDF:_B};const e=O,YA=nA,{extractSignature:$B,getMessageFromSignature:An,getClientCertificate:Bn,checkForSubFilter:nn,preparePDF:En,authenticateSignature:Mn,sortCertificateChain:Qn,isCertsExpired:In}=oA,{extractCertificatesDetails:wn}=bA,Dn=(g,D,C)=>{const V=An(g),{certificates:c,rawCapture:{signature:T,authenticatedAttributes:N,digestAlgorithm:Q}}=V,Y=e.asn1.derToOid(Q),h=e.pki.oids[Y].toLowerCase(),U=e.asn1.create(e.asn1.Class.UNIVERSAL,e.asn1.Type.SET,!0,N),b=Bn(c),j=e.md[h].create().update(e.asn1.toDer(U).data).digest().getBytes();if(!b.publicKey.verify(j,T))throw new YA("Wrong authenticated attributes",YA.VERIFY_SIGNATURE);const k=e.pki.oids.messageDigest,m=N.find(q=>e.asn1.derToOid(q.value[0].value)===k).value[1].value[0].value,H=e.md[h].create().update(D.toString("latin1")).digest().getBytes()===m,J=Qn(c),a=wn(J),W=Mn(J),p=In(J);return{verified:H&&W&&!p,authenticity:W,integrity:H,expired:p,meta:{certs:a,signatureMeta:C}}};var gn=g=>{const D=En(g);nn(D);try{const{signatureStr:C,signedData:V,signatureMeta:c}=$B(D),T=V.map((N,Q)=>Dn(C[Q],N,c[Q]));return{verified:T.every(N=>N.verified===!0),authenticity:T.every(N=>N.authenticity===!0),integrity:T.every(N=>N.integrity===!0),expired:T.some(N=>N.expired===!0),signatures:T}}catch(C){return{verified:!1,message:C.message,error:C}}};const ZA=gn,{getCertificatesInfoFromPDF:Cn}=bA;Object.assign(ZA,{getCertificatesInfoFromPDF:Cn});var Nn=ZA;const Gn=PA(Nn),Vn={style:{flex:"0 0 100%"}},Tn={"mx-auto":"","max-w-600px":""},Fn={"font-bold":""},Un={key:1},tn={key:0,style:{flex:"0 0 100%"},"mt-5":"",flex:"","flex-col":"","gap-4":""},cn={"mb-2":"","font-bold":""},dn=RA({__name:"pdf-signature-checker",setup(g){const D=$([]),C=$("idle"),V=$(null);async function c(T){V.value=T;const N=await T.arrayBuffer();C.value="loading";try{const{signatures:Q}=Gn(N);D.value=Q,C.value="parsed"}catch{D.value=[],C.value="error"}}return(T,N)=>{const Q=BB,Y=nB,h=EB,U=NB;return Z(),s(FA,null,[y("div",Vn,[y("div",Tn,[x(Q,{title:"Drag and drop a PDF file here, or click to select a file",accept:".pdf",onFileUpload:c}),o(V)?(Z(),_A(Y,{key:0,"mt-4":"",flex:"","gap-2":""},{default:u(()=>[y("div",Fn,L(o(V).name),1),y("div",null,L(o(DB)(o(V).size)),1)]),_:1})):AA("",!0),o(C)==="error"?(Z(),s("div",Un,[x(h,{"mt-4":""},{default:u(()=>[$A(" No signatures found in the provided file. ")]),_:1})])):AA("",!0)])]),o(C)==="parsed"&&o(D).length?(Z(),s("div",tn,[(Z(!0),s(FA,null,AB(o(D),(b,j)=>(Z(),s("div",{key:j},[y("div",cn," Signature "+L(j+1)+" certificates : ",1),x(U,{signature:b},null,8,["signature"])]))),128))])):AA("",!0)],64)}}});export{dn as default};
